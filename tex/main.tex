...
\lstset{linewidth=1.2\linewidth}
...
Python posiada interaktywnego shella o~nazwie IDLE.~Interaktywność oznacza, że reaguje on na~wszystkie twoje polecenia zaraz po ich wpisaniu (nieinteraktywny tryb pracy to taki w~którym wpierw wpisujesz wszystkie polecenia do~pliku, a~dopiero potem są one wykonywane po kolei). Shell\footnote{Shell jest przez niektórych nazywany również ,,powłoką''. Zabawne, prawda?} to miejsce gdzie wpisujesz polecenia - zgłasza się znakiem \lstinline{>>>} nazywanym znakiem zachęty (lub promptem).
...
% Note from Matt: there are different keyboard layouts. For example Mac keyboard has the backtick next to the left shift
...
{\bf Uwaga!} Ten program działa tylko z~Pythonem w~wersji 3, więc jeśli masz zainstalowanego Pythona 2.6 lub jakąkolwiek inną wersję z~serii 2.x, program nie będzie działał. Świadczy o~tym błąd: \lstinline{NameError: name is not defined}.
...
Kiedy wpisujesz program, powinieneś raz na~jakiś czas zapisywać jego kod. W~ten sposób, jeśli komputer przypadkowo się wyłączy lub niechcący wyjdziesz z~IDLE, zgubi się tylko część tekstu. Wciskając Ctrl-S, szybko zapiszesz plik bez użycia myszki.
...
Spójrzmy teraz na~program linia po linii żeby zobaczyć co robi. Zacznijmy od pierwszej.
...
Funkcja o~nazwie input() nie potrzebuje (w~przeciwieństwie do~print()) od programu żadnych danych, dlatego nie umieściliśmy nic pomiędzy jej nawiasami.
...
%\lstset{title={Funkcja randint() losuje liczby}}
\bl{}
\begin{lstlisting}
>>> import random
>>> random.randint(1, 20)
...
%\lstset{title={Funkcja randint() i różne zakresy losowania}}
\begin{lstlisting}
>>> random.randint(1, 4)
3
>>> random.randint(1, 4)
...
Wartości logiczne (boolowskie) to typ danych który może przyjmować tylko dwie wartości: Prawda (ang. True) lub Fałsz (ang. False). Te wartości muszą być pisane z~dużej litery i~nie są stringami, czyli {\bf nie} mogą ,,znajdować się w~cudzysłowach''. Będziemy używać wartości logicznych z~operatorami porównania do~tworzenia warunków. Wszystko jest wyjaśnione poniżej.
...
\ldots{}i gracz dostaje tylko cztery próby zamiast sześciu.
\end{minipage}
...
Widzimy że wyrażenie \lstinline{int('42')} zwróci liczbę całkowitą 42, i~że \lstinline{int(42)} zwróci dokładnie to samo (chociaż zamiana liczby całkowitej na~liczbę całkowitą nie ma przecież sensu). Ale pomimo tego że do~funkcji \lstinline{int()} da się przekazać stringa, nie oznacza to że może on zawierać cokolwiek. Na przykład przekazanie do~funkcji \lstinline{int()} stringa 'czesc' spowoduje błąd. Tekst przekazywany do~\lstinline{int()} musi składać się z~cyfr.
...
Linie od 22 do~26 sprawdzają czy strzał gracza był większy bądź dokładnie równy wylosowanej liczbie.
...
Instrukcja break składa się tylko ze słowa \lstinline{break} bez żadnych warunków ani dwukropków.
...
%TODO dokończyć i skonsultować się w sprawie czerstwych dowcipów
...
Poniżej znajduje się kod źródłowy gry Smocze Królestwo. Przepisywanie kodu to świetny sposób żeby przyzwyczaić się do niego, ale jeśli nie chcesz przepisywać, możesz ściągnąć go na stronie książki. %TODO
Na stronie WWW znajdują się instrukcje które powiedzą ci jak ściągnąć i otworzyć plik z kodem źródłowym. %TODO diff
...
\bnl{caption=smoki.py}
\lstinputlisting{python/smoki.py}

\section{Jak działa ten program}

Spójrzmy na kod gry bardziej szczegółowo.

\bnl{}
\lstinputlisting[linerange=1-2]{python/smoki.py}

Na początku mamy dwa wyrażenia \lstinline{import}. Importujemy moduł random (liczby losowe) tak, jak zrobiliśmy to wcześniej w Zgaduj-Zgaduli. W Smoczym Królestwie będziemy używać też funkcji związanych z czasem (ang. time), które dostarczane są przez moduł time - dlatego jego też importujemy.

\subsection{Definiowanie funkcji pokazWstep()}

\bnl{}
\lstinputlisting[linerange=4-9]{python/smoki.py}

%TODO rysunek 6.1

Rysunek \ref{} pokazuje nowy typ instrukcji, {\bf instrukcję def}. Instrukcja def składa się ze słowa kluczowego \lstinline{def} oraz nazwy funkcji z nawiasami, oraz dwukropka (znaku : ). Za instrukcją znajduje się blok kodu nazywany {\bf ciałem funkcji}.

\subsection{Instrukcje def}

Instrukcja def nie jest wywołaniem funkcji o nazwie \lstinline{pokazWstep()}. Oznacza ona że tworzymy, lub {\bf definiujemy} taką funkcję, którą możemy później wywołać w programie. Po tym jak zdefiniujemy funkcję, możemy ją wywoływać w ten sposób jak robimy to ze wszystkimi innymi funkcjami takimi, jak \lstinline{print()}. Kiedy wywołamy tą funkcję, wykona się kod z ciała funkcji.

Podobnie, kiedy tworzymy zmienne, mówimy że je definiujemy. Kod  \lstinline{foo = 42} definiuje zmienną \lstinline{foo}.

Pamiętaj że instrukcja \lstinline{def} nie sprawia że kod wykonuje się od razu. Mówi tylko jaki kod wykona się kiedy wywołamy funkcję \lstinline{pokazWstep()} w dalszej części programu. Kiedy wykonanie programu dojdzie do instrukcji  \lstinline{def}, program przeskakuje na koniec ciała funkcji. Wskoczymy do środka ciała funkcji kiedy wywołamy funkcję \lstinline{pokazWstep()}. Spowoduje to że wszystkie instrukcje \lstinline{print()} wewnątrz ciała funkcji zostaną wykonane po kolei. Wywołujemy więc tą funkcję kiedy chcemy wyświetlić użytkownikowi wstęp "Jesteś w krainie pełnej smoków...".

Kiedy wywołamy funkcję \lstinline{pokazWstep()}, wykonanie programu przeskoczy do początku funkcji w linii 5. Kiedy program dojdzie do końca ciała funkcji, program przeskoczy do linii w której funkcja została wykonana.

Zanim wyjaśnimy główną część programu, dowiemy się co robią poszczególne funkcje które program wykonuje. Może być trochę trudno uczyć się programu w innej kolejności niż się wykonuje, ale pamiętaj że kiedy definiujemy funkcje, siedzą gdzieś z boku i czekają na wywołanie.

\subsection{Funkcja wybierzJaskinie()}

\bnl{}
\lstinputlisting[linerange=11-11]{python/smoki.py}

W tej linii definiujemy inną funkcję o nazwie \lstinline{wybierzJaskinie()}. Kod tej funkcji będzie pytał użytkownika o jaskinię do której chce wejść.

\bnl{}
\lstinputlisting[linerange=12-13]{python/smoki.py}

W funkcji \lstinline{wybierzJaskinie()} tworzymy nową zmienną o nazwie \lstinline{jaskinia} i przechowujemy w niej pusty tekst. Później zaczynamy pętlę \lstinline{while}. 
